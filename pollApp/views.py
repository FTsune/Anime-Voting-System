...
def index(request):
    ...
    if selected_season_id:
        try:
            ...
            latest_question_list = Question.objects.filter(seasons=selected_season).order_by('-pub_date')[:10]
        except Season.DoesNotExist:
            print("Invalid Season ID or Season not associated with the selected year.")
    else:
        latest_question_list = Question.objects.filter(seasons__year=selected_year).order_by('-pub_date')[:10]
